...
{
	int ch;				// user command
	// malloc the memory for waitList handle
	List *waitList = (List*)malloc(sizeof(List));
	
	debugMode = FALSE;		// initialize to FALSE

	PrintAuthorInformation();

	// check arguments for "-d" flag / debug mode
	if (argc < 3) {
		int i = 0;		// loop counter
		for (; i < argc; i++) {
			if (strcmp("-d", argv[i]) == 0) {
				debugMode = TRUE;		//  "-d" flag provided, turn on debugMode
				printf("Debug mode is turned on.\n\n");
			}
		}
	}
	else {		// wrong command, display error message
		printf("Too many arguments were given.\n");
		printf("Usage: %s <\"-d\"optional for debugMode>\n", argv[0]);
	}

	// initialize the list handle
	waitList = InitializeList(waitList);
